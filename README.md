...
- **Phrase editor** — a tracker-style grid edited in place: notes, instruments and FX chosen by name with fuzzy matching, block selection over steps and columns (copy/paste/clear/transpose), insert/delete step, ±1/±12 transpose, per-step and whole-phrase clipboard, a 40-level undo, an optional QWERTY piano mode for typing notes, and a **🔍 INS** inspector that pops the full decoded parameters of the instrument under the cursor.
- **Table editor** — the M8's 256 instrument/FX tables, opened from the pattern editor: a chip list marks tables holding data, and each table edits as a 16-step grid (transpose, volume, three FX columns) with hex entry and undo, FX names resolved through the owning instrument.
...
- **Output effects** — thirty-two GPU effects over the mirror, from CRT, glitch and VHS through the SIGNAL//ROT signal path (composite decode, tape rot, sync damage, bent enhancer, feedback rescan trails), a **Lottes CRT** (Timothy Lottes' public-domain shader: gaussian beam per scanline in linear light, grille/TV/VGA shadow masks) and **background feedback** — the letterbox bars become a live enhancer feedback loop fed by the screen's own glow, with drift and screen echo to keep it moving and a picture inset so the display floats over the churn — with twenty-three presets, a 🎲 Random look roller, per-parameter audio routing (up to three routes per effect, each with its own source, target and depth) and persistent settings, in a single WebGL pass, living in a slide-out drawer beside the mirror. **⇩/⇪ Look** saves and loads the whole effects state as a small JSON file to trade. Output sizes include **custom dimensions** for wide banners and capture rigs, and a **Text** field stamps your own title over the screen (demo or connected device).
